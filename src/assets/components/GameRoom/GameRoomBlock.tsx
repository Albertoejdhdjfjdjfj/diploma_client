...
const GameRoomBlock = ({game_room}:{game_room:GameRoom}) => {
     ...
     return (
          <div className='game_room'>
               <span>{players+observers}/{MAX_NUMBER_PLAYERS}</span>
               <p className='game_room_name'>{game_room.name}</p>
               <div className='participants'>
                    {game_room.players.map((player:Player)=><p>{player.nickname}</p>)}
                    {game_room.observers.map((observer:Player)=><p>{observer.nickname}</p>)}    
               </div>
               <p className='game_room_creator'>{game_room.creator.nickname}</p>
               <div>
                    <button><img alt='start game' src={gamepad}/></button>
                    <button><img alt='join to game' src={plus}/></button>
               </div>
           </div>
     );
}
